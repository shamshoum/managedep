...
/**
 * This function accepts a file as a parameter
 * and returns an array of required dependencies inside the file
 * @param file
 * @returns {Array}
 */
exports.checkFileDep = function (file) {

  var dependencies = [];
  var commentBlock = false; // Flag for block of comments
  var singleComment = false; // Flag for single line comments
  ...
  fs.readFileSync(file).toString().split('\n').forEach(function (line) {
    ...
    if (commentBlock) {
      ...
      var dep = line.match(/require\((.*?)\)/gmi);
      if (dep && dep.length > 0) {
        if (line.indexOf('//') != -1) {
          singleComment = true;
        }
        dep.forEach(function (dependency) {
          if (singleComment) {
            if (line.indexOf(dependency) < line.indexOf('//')) {
